...
void QuoteDemoLv2::Run()
{
    ...
    quote_api_->Login("10.10.184.37", 8118, "123", "123");
...
// 深交所快照行情
void QuoteDemoLv2::OnLv2SnapSze(EMQSzeSnap *snap)
{
...
// 深交所逐笔合并行情
void QuoteDemoLv2::OnLv2TickSze(EMQSzeTick *tick)
{
...
// 深交所指数行情
void QuoteDemoLv2::OnLv2IndexSze(EMQSzeIdx *idx)
{
...
// 深交所债券快照行情
void QuoteDemoLv2::OnLv2BondSnapSze(EMQSzeBondSnap *bond_snap)
{
...
// 深交所债券逐笔合并行情
void QuoteDemoLv2::OnLv2BondTickSze(EMQSzeBondTick *bond_tick)
{
...
// 深交所建树行情
void QuoteDemoLv2::OnLv2TreeSze(EMQSzeTree *tree)
{
...
// 上交所快照行情
void QuoteDemoLv2::OnLv2SnapSse(EMQSseSnap *snap)
{
    auto ts = quote_api_->GetPacketHardwareRXTs(snap);
    char cache[kCacheSize];
    ...
    sprintf(cache,
            ...
            snap->m_total_ask_quantity, snap->m_total_ask_weighted_avg_price, snap->m_IOPV,
...
// 上交所逐笔合并行情
void QuoteDemoLv2::OnLv2TickSse(EMQSseTick *tick)
{
    auto ts = quote_api_->GetPacketHardwareRXTs(tick);
    char cache[kCacheSize] = {0};
...
// 上交所指数行情
void QuoteDemoLv2::OnLv2IndexSse(EMQSseIdx *idx)
{
    auto ts = quote_api_->GetPacketHardwareRXTs(idx);
    char cache[1024];
...
// 上交所债券快照行情
void QuoteDemoLv2::OnLv2BondSnapSse(EMQSseBondSnap *bond_snap)
{
    auto ts = quote_api_->GetPacketHardwareRXTs(bond_snap);
    char cache[kCacheSize];
...
// 上交所债券逐笔行情
void QuoteDemoLv2::OnLv2BondTickSse(EMQSseBondTick *bond_tick)
{
    auto ts = quote_api_->GetPacketHardwareRXTs(bond_tick);
    char cache[kCacheSize];
...
// 上交所建树行情
void QuoteDemoLv2::OnLv2TreeSse(EMQSseTree *tree)
{
    auto ts = quote_api_->GetPacketHardwareRXTs(tree);
    char cache[kCacheSize] = {0};
    sprintf(cache,
