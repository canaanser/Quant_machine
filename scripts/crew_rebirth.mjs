...
- Agent 侧只做一件：旧实例进 \`failedThreadIds\`（\`--apply\` 已自动写）。
  **窗口标题由老板定——Agent 不主动改任何窗口标题**（含 \`·旧（待归档）\` 这种后缀）：标题是老板在侧边栏认人的锚。
...
function apply(name, tid) {
  ...
  console.log("   Agent 只做「进 failedThreadIds」（已自动）——**不代按归档、也不主动改任何窗口标题**："
    + "标题是老板认人的锚；新实例没验过之前，旧窗是唯一回退面。");
