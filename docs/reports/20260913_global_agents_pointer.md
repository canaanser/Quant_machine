...
6. **边界（`codex-套件` 09:04 报、我已复验）**：只对**新开的会话**生效；**已在跑的会话不追溯加载**——它那条会话 09-12 21:08 就开了，启动注入实测为**无**，属预期，不是缺陷。
...
| **影响面** | **所有线的新会话**：启动多加载这一页（约 230 字节）并因此知道真源在哪；**已在跑的会话不受影响、也不追溯**（等它下次新开窗口/换实例） |
