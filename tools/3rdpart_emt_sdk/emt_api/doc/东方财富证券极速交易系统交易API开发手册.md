<font size="6">**东方财富证券极速交易系统交易API开发手册(Ver2.27.0)**</font><br>
...
<table><tbody>
    ...
    <tr>
        ...
        <td>
            新增现货查询客户可用接入点ID接口:QueryEndpointId <br>
    ...
    <tr>
        <td>20250926</td><td>2.25.0</td>
        <td>
            新增现货黑白名单证券查询:QueryBlackWhiteListByPage <br>
            现货资金划拨推送、资金划拨流水查询优化 <br>
            部分错误码描述更新 <br>
        </td>
    </tr>
    <tr>
        <td>20251226</td><td>2.26.0</td>
        <td>
            交易: <br>
              优化登录接口错误信息返回 <br>
              两融标的查询接口新增保证金率字段 <br>
            行情: <br>
              新增北交所极速行情API <br>
              完善 Level-2 极速行情接口报错信息 <br>
        </td>
    </tr>
    <tr>
        <td>20260413</td><td>2.27.0</td>
        <td>
            两融保证金调拨请求CreditMarginAvlTransfer<br>
            支持两融保证金调拨查询QueryCreditMarginAvlTransfer<br>
            现货资产查询QueryAsset支持当日买入/卖出成交金额、成交手续费字段 <br>
            错误码列表更新 <br>
        </td>
    </tr>
</table>
...
|[QueryBlackWhiteListByPage](#queryblackwhitelistbypage) |[OnQueryBlackWhiteListByPage](#onqueryblackwhitelistbypage) |分页查询黑白名单|
|[CreditMarginAvlTransfer](#creditmarginavltransfer) |[OnCreditMarginAvlTransfer](#oncreditmarginavltransfer) |融资融券两地分仓保证金调拨请求|
|[QueryCreditMarginAvlTransfer](#querycreditmarginavltransfer) |[OnQueryCreditMarginAvlTransfer](#onquerycreditmarginavltransfer) |融资融券两地分仓保证金调拨查询请求|
...
### 3.2.63. <a id="queryendpointid">查询客户可用接入点ID</a>

| 接口名称 | QueryEndpointId |                                                       |                                              |
| :------- |:----------------| :---------------------------------------------------- | :------------------------------------------- |
| 功能描述 | 查询客户可用接入点ID     |                                                       |                                              |
|          | <b>名称</b>       | <b>类型及长度</b>                                     | <b>描述</b>                                  |
| 入参     | 需要查询的可用接入点条件    | [EMTQueryEndPointIdReq](#emtqueryendpointidreq) * | 类型参见入参结构体定义                       |
|          | 会话号             | uint64_t  session_id                                  | 资金账户对应的session_id,登录时得到          |
|          | 请求序号            | int       request_id                                  | 用于用户定位查询响应的ID，由用户自定义       |
| 出参     | 函数返回值           | int                                                   |                                              |
|          | 查询客户可用接入点ID响应   | [EMTQueryEndpointIdRsp](#emtqueryendpointidrsp)*  | 类型参见出参结构体定义；异步调用回调函数返回 |
|          | 错误响应结构体         | [EMTRspInfoStruct](#emtrspinfostruct)*                | 类型参见出参结构体定义                       |
|          | 请求序号            | int       request_id                                  | 此消息响应函数对应的请求ID                   |
|          | 会话号             | uint64_t  session_id                                  | 资金账户对应的session_id,登录时得到          |
| 备注信息 |                 |                                                       |                                              |
...
### 3.2.65. <a id="queryblackwhitelistbypage">分页请求查询查询黑白名单信息</a>

| 接口名称 | QueryBlackWhiteListByPage      |                                                       |                                              |
| :------- | :----------------------- | :---------------------------------------------------- | :------------------------------------------- |
| 功能描述 | 分页请求查询查询黑白名单信息 |                                                       |                                              |
|          | <b>名称</b>              | <b>类型及长度</b>                                     | <b>描述</b>                                  |
| 入参     | 需要分页查询查询黑白名单信息的条件 | [EMTQueryBlackWhiteListByPageReq](#emtqueryblackwhitelistbypagereq) * | 类型参见入参结构体定义                       |
|          | 会话号                   | uint64_t  session_id                                  | 资金账户对应的session_id,登录时得到          |
|          | 请求序号                 | int       request_id                                  | 用于用户定位查询响应的ID，由用户自定义       |
| 出参     | 函数返回值               | int                                                   |                                              |
|          | 黑白名单信息       | [EMTQueryBlackWhiteListRsp](#emtqueryblackwhitelistrsp)*  | 类型参见出参结构体定义；异步调用回调函数返回 |
|         |请求到的最大数量             |int64_t   req_count                   ||
|         |分页请求的当前回报数量       |int64_t   rsp_sequence               ||
|         |当前报单信息所对应的查询索引  |int64_t   query_reference              |需要记录下来，在进行下一次分页查询的时候需要用到|
|         |请求序号                    |int       request_id                   |此消息响应函数对应的请求ID|
...
### 3.2.66. <a id="creditmarginavltransfer">融资融券两地分仓保证金调拨请求</a>

|接口名称 |CreditMarginAvlTransfer     |                          ||
|:-------|:--|:--|:--|
|功能描述 |融资融券两地分仓保证金调拨请求                           |                             ||
|         |<b>名称</b>   |<b>类型及长度</b>                            |<b>描述</b>|
|入参     |两地分仓保证金划拨请求|[EMTMarginAvlTransferReq](#emtmarginavltransferreq)*  |类型参见入参结构体定义|
|         |会话号        |uint64_t  session_id                         |资金账户对应的session_id,登录时得到|
|出参     |函数返回值     |int                                    ||
|         |两地分仓保证金内转流水通知|[EMTMarginAvlTransferNotice](#emtmarginavltransfernotice)* |类型参见出参结构体定义；异步调用回调函数返回|
|         |错误响应结构体 |[EMTRspInfoStruct](#emtrspinfostruct)*              |类型参见出参结构体定义|
|         |会话号        |uint64_t  session_id                                 |资金账户对应的session_id,登录时得到|
|备注信息  |              |                                            |||

### 3.2.67. <a id="querycreditmarginavltransfer">融资融券两地分仓保证金调拨查询请求</a>

|接口名称 |QueryCreditMarginAvlTransfer     |                          ||
|:-------|:--|:--|:--|
|功能描述 |融资融券两地分仓保证金调拨查询请求                           |                             ||
|         |<b>名称</b>   |<b>类型及长度</b>                            |<b>描述</b>|
|入参     |两地分仓保证金调拨流水查询请求|[EMTQueryMarginAvlTransferLogReq](#emtquerymarginavltransferlogeq)*  |类型参见入参结构体定义|
|         |会话号        |uint64_t  session_id                         |资金账户对应的session_id,登录时得到|
|         |请求序号      |int       request_id                         |用于用户定位查询响应的ID，由用户自定义|
|出参     |函数返回值     |int                                    ||
|         |两地分仓保证金内转流水通知|[EMTMarginAvlTransferNotice](#emtmarginavltransfernotice)* |类型参见出参结构体定义；异步调用回调函数返回|
...
|接入点ID          |ep_id          |uint8_t        |N| 仅具备指定接入点ID权限的客户填写                                               |
...
|两地分仓信用额度划拨请求|EMTQuotaTransferReq||||
|:----------------|:----------------|:----------------|:------------|:-----|
|<b>变量名称</b>   |<b>标识</b>  |<b>类型及长度</b> |<b>必要</b>  |<b>描述</b>|
...
### 3.3.30. <a id="emtqueryendpointidreq">EMTQueryEndPointIdReq</a>
...
| 查询客户可用接入点ID请求结构体 | EMTQueryEndPointIdReq |                   |             |                                                         |
| :--------------------------------- | :---------------------- | :---------------- | :---------- | :------------------------------------------------------ |
| <b>变量名称</b>                    | <b>标识</b>             | <b>类型及长度</b> | <b>必要</b> | <b>描述</b>                                             |
...
### 3.3.32. <a id="emtqueryblackwhitelistbypagereq">EMTQueryBlackWhiteListByPageReq</a>
...
| 分页查询黑白名单信息的请求结构体 | EMTQueryBlackWhiteListByPageReq |                   |             |                                                         |
| :--------------------------------- | :---------------------- | :---------------- | :---------- | :------------------------------------------------------ |
| <b>变量名称</b>                     | <b>标识</b>             | <b>类型及长度</b>  | <b>必要</b> | <b>描述</b>                                             |
| 查询的黑白名单类型                   | is_black                | bool              | Y           | false-白名单<br>true-黑名单                             |
| EMT_MAREKT_TYPE                    | market                  | enum              | Y           | 参考字典定义<br>[EMT_MARKET_TYPE](#emt_market_type)      |
| 需要查询的记录数                    | req_count                | int64_t           | Y           |                                                         |
| 查询索引                           | reference                | int64_t           | Y           | 上一次收到的查询订单结果中带回来的索引，如果是从头查询，请置0 |
| 预留字段                           | reserved                 | char[12]          | N           |                                                         |

### 3.3.33. <a id="emtmarginavltransferreq">EMTMarginAvlTransferReq</a>
...
|两地分仓保证金划拨请求|EMTMarginAvlTransferReq||||
|:----------------|:----------------|:----------------|:------------|:-----|
|<b>变量名称</b>   |<b>标识</b>  |<b>类型及长度</b> |<b>必要</b>  |<b>描述</b>|
|内转编号      |emt_id       |uint64_t         |Y            ||
|资金账户代码      |fund_account     |char[16]             |Y            ||
|金额              |amount          |double           |Y            ||
|内转类型          |transfer_type   |enum             |Y            |参见字典定义<br>[EMT_MARGINAVL_TRANSFER_TYPE](#emt_marginavl_transfer_type)|
|备注信息  ||       ||||

### 3.3.34. <a id="emtquerymarginavltransferlogeq">EMTQueryMarginAvlTransferLogReq</a>
...
|两地分仓保证金调拨流水查询请求|EMTQueryMarginAvlTransferLogReq||||
|:----------------|:----------------|:----------------|:------------|:-----|
|<b>变量名称</b>   |<b>标识</b>  |<b>类型及长度</b> |<b>必要</b>  |<b>描述</b>|
|内转编号      |emt_id       |uint64_t         |Y            ||
|备注信息  ||       ||||
...
| 算法类型         |algorithm_type          |uint8_t            |N            | 无需用户填写                                                                                            |
| 预留空间         |reserved                |char            |N|                                                                                                   |
| 接入点ID        |ep_id |uint8_t |N| 该笔订单的接入点ID                                                                                        |
...
struct EMTQueryAssetRsp
{
    ...
    ///累计买入成交交易费用（仅限现货账户/期权账户，现货账户含ETF申购等业务占用资金，信用账户暂不可用）
    double fund_buy_fee;
    ///累计卖出成交证券所得资金（仅限现货账户/期权账户，现货账户含ETF申购等业务占用资金，信用账户暂不可用）
    double fund_sell_amount;
    ///累计卖出成交交易费用（仅限现货账户/期权账户，现货账户含ETF申购等业务占用资金，信用账户暂不可用户）
...
|累计买入成交证券占用资金  |fund_buy_amount            |double       |仅限现货账户/期权账户，现货账户含ETF申购等业务占用资金，信用账户暂不可用 |
|累计买入成交交易费用      |fund_buy_fee               |double       |仅限现货账户/期权账户，现货账户含ETF申购等业务占用资金，信用账户暂不可用 |
|累计卖出成交证券所得资金  |fund_sell_amount           |double       |仅限现货账户/期权账户，现货账户含ETF申购等业务占用资金，信用账户暂不可用 |
|累计卖出成交交易费用      |fund_sell_fee              |double       |仅限现货账户/期权账户，现货账户含ETF申购等业务占用资金，信用账户暂不可用 |
...
### 3.4.52. <a id="emtqueryendpointidrsp">EMTQueryEndpointIdRsp</a>
...
| 查询客户可用接入点ID响应结构体 | EMTQueryEndpointIdRsp |                   |                                                              |
|:-----------------| :---------------------- | :---------------- | :----------------------------------------------------------- |
| <b>变量名称</b>      | <b>标识</b>             | <b>类型及长度</b> | <b>描述</b>                                                  |
...
### 3.4.54. <a id="emtqueryblackwhitelistrsp">EMTQueryBlackWhiteListRsp</a>
...
| 查询黑白名单信息的响应结构体           | EMTQueryBlackWhiteListRsp |              |                                                              |
|:------------------------------|:----------------------------|:-------------| :----------------------------------------------------------- |
| <b>变量名称</b>                   | <b>标识</b>                   | <b>类型及长度</b> | <b>描述</b>                                                  |
| 黑白名单类型                      | is_black                    | bool         | false-白名单<br>true-黑名单                                |
| 证券市场                          | market                      | enum         | 参考字典定义<br>[EMT_MARKET_TYPE](#emt_market_type)      |
| 证券代码                          | ticker                      | char[16]     |                                                              |

### 3.4.55. <a id="emtmarginavltransfernotice">EMTMarginAvlTransferNotice</a>
...
|两地分仓保证金内转流水通知|EMTMarginAvlTransferNotice||||
|:----------------|:----------------|:----------------|:------------|:-----|
|<b>变量名称</b>   |<b>标识</b>  |<b>类型及长度</b> |<b>必要</b>  |<b>描述</b>|
|内转编号      |emt_id       |uint64_t         |Y            ||
|内转类型          |transfer_type   |enum             |Y            |参见字典定义<br>[EMT_MARGINAVL_TRANSFER_TYPE](#emt_marginavl_transfer_type)|
|金额              |amount          |double           |Y            ||
|操作结果          |oper_status     |enum             |Y            |参见字典定义<br>[EMT_MARGINAVL_OPER_STATUS](#emt_marginavl_oper_status)|
|操作时间          |transfer_time   |uint64_t         |Y            | |
|备注信息  ||       ||||
...
### 4.1.55. <a id="onqueryyesterdayasset">OnQueryYesterdayAsset</a>
...
### 4.1.66. <a id="onqueryendpointid">OnQueryEndpointId</a>
...
### 4.1.68. <a id="onqueryblackwhitelistbypage">OnQueryBlackWhiteListByPage</a>
...
### 4.1.69. <a id="oncreditmarginavltransfer">OnCreditMarginAvlTransfer</a>
...
### 4.1.70. <a id="onquerycreditmarginavltransfer">OnQueryCreditMarginAvlTransfer</a>
...
|EMT_STK_QUOTA_TRANSFER_IN            |1    |信用额度划入|
...
### 5.1.38. <a id="emt_marginavl_transfer_type">保证金调拨方向类型-EMT_MARGINAVL_TRANSFER_TYPE</a>

|枚举    |取值    |定义说明|
|:------|:-------|:--------|
|EMT_MARGINAVL_TRANSFER_OUT           |0    |保证金划出|
|EMT_MARGINAVL_TRANSFER_IN            |1    |保证金划入|
|EMT_MARGINAVL_TRANSFER_UNKNOWN       |4    |未知类型|

### 5.1.39. <a id="emt_marginavl_oper_status">融资融券保证金调拨操作结果-EMT_MARGINAVL_OPER_STATUS</a>

|枚举    |取值    |定义说明|
|:------|:-------|:--------|
|EMT_MARGINAVL_OPER_PROCESSING            |0    |EMT已收到，正在处理中|
|EMT_MARGINAVL_OPER_SUCCESS               |1    |成功|
|EMT_MARGINAVL_OPER_FAILED                |2    |失败|
|EMT_MARGINAVL_OPER_SUBMITTED             |3    |已提交到对应核心处理|
|EMT_MARGINAVL_OPER_UNKNOWN               |4    |未知|