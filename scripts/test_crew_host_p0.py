...
# ── ①-2 行首判据的「剥信头」防护（老板 2026-09-13 08:5x 定）───────────────────
#   三条判别性用例（缺一不算）：**带头 / 不带头 / 正文里引用** 必须分别给对答案。
#   为什么必须三条：只测"带头"会把判据改成"包含"，只测"不带头"测不出这处防护。
HDR = "【PLT-006 · 2026-09-13 08:44】"
check("① 带工具信头（信头挡在正文最前）的公告 → **仍判 P0**",
      ch.is_priority_row({"from": "codex-看板编辑", "body": HDR + " 【公告 N-A5EB】禁令解除，开始干活"}))
check("② 不带信头的公告 → **仍判 P0**（原行为不许回归）",
      ch.is_priority_row({"from": "codex-看板编辑", "body": "【公告 N-A5EB】禁令解除，开始干活"}))
check("③ 正文里**引用**一句【公告（不在行首）→ 判据不受影响：**不算 P0**",
      not ch.is_priority_row({"from": "codex-套件", "body": HDR + " 你那条【公告 N-A5EB】我已经回过了"}))
check("③-a 〔代答〕被信头挡住时**仍算代答**（水位不吃它）",
      ch.strip_tool_prefix(HDR + " 〔代答〕我在，本尊不在。").startswith("〔代答〕"))
check("③-b 信头**只剥一层**、只剥我方那一款（正文里第二段【…】原样保留）",
      ch.strip_tool_prefix("【无卡 · 2026-09-13 06:51】 你那条【公告 N-A5EB】我收到了") == "你那条【公告 N-A5EB】我收到了")
check("③-c **公告标题不许被当成信头剥掉**（无差别剥会让真公告掉出 P0——实测这条曾红）",
      ch.strip_tool_prefix("【公告 N-A5EB】禁令解除") == "【公告 N-A5EB】禁令解除")
