...
* API支持多账户登录，不同账户调用多次Login即可，账号之间使用登录返回的sessionid进行区分。注意API为单例模式，同一进程内只有一个API实例，重复调用CreateTraderApi返回同一个句柄，无法通过多次调用CreateTraderApi创建多个TraderApi来实现单进程内多账号登录。
