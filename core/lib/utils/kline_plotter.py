...
def plot_fenshi(minute_df, stock_code="", stock_name="", prev_close=None,
                title=None):
    """分时图（东方财富样式，2026-09-06 老板拍板）
    要点：白线=分时价、黄线=均价线、灰虚线=昨收、下挂量柱
    午休处理：交易分钟用序号轴（去掉午休占位，11:30 直接接 13:00，线连续不画空隙）
    minute_df: DataFrame，index=datetime(分钟)，含 open/high/low/close/amount/volume
    prev_close: 昨收（None 用开盘价）
    """
    if minute_df is None or minute_df.empty or 'close' not in minute_df:
        return go.Figure()
    df = minute_df.copy().sort_index()
    if prev_close is None:
        prev_close = float(df['open'].iloc[0])
    # 只保留交易时段分钟（9:30-11:30 / 13:00-15:00），天然无午休点
    hm = df.index.strftime('%H:%M')
    mask = ((hm >= '09:30') & (hm <= '11:30')) | ((hm >= '13:00') & (hm <= '15:00'))
    df = df[mask]
    if df.empty:
        return go.Figure()
    price = df['close']
    pct = (price / prev_close - 1) * 100
    # 序号轴：午休不占位 → 11:30 与 13:00 直接相邻
    pos = list(range(len(df)))
    time_labels = df.index.strftime('%H:%M')

    # 均价线 = 累计成交额/累计成交量（仅盘中累计）
    has_amt = 'amount' in df.columns
    has_vol = 'volume' in df.columns
    if has_amt and has_vol:
        cum_amt = df['amount'].fillna(0).cumsum()
        cum_vol = df['volume'].fillna(0).cumsum().replace(0, float('nan'))
        avg = (cum_amt / cum_vol).round(3)
    else:
        avg = None
    if has_vol:
        vol = df['volume'].fillna(0)
        vol_colors = ['#E74C3C' if c >= o else '#2ECC71'
                      for c, o in zip(df['close'], df['open'])]
    else:
        vol = pd.Series(0, index=df.index)
        vol_colors = ['#E74C3C'] * len(df)

    fig = make_subplots(rows=2, cols=1, shared_xaxes=True,
                        row_heights=[0.72, 0.28], vertical_spacing=0.02)
    fig.add_trace(go.Scatter(
        x=pos, y=price, mode='lines', name='分时价',
        line=dict(color='#1a66cc', width=1.3),
        hovertemplate='<b>%{customdata[0]}</b> 价 %{y:.2f} (%{customdata[1]:+.2f}%)<extra></extra>',
        customdata=list(zip(time_labels, pct.round(2)))), row=1, col=1)
    if avg is not None:
        fig.add_trace(go.Scatter(
            x=pos, y=avg, mode='lines', name='均价',
            line=dict(color='#e6b800', width=1.3),
            hovertemplate='<b>%{customdata}</b> 均价 %{y:.2f}<extra></extra>',
            customdata=time_labels), row=1, col=1)
    fig.add_hline(y=prev_close, line=dict(color='#999', dash='dash', width=1), row=1, col=1)
    fig.add_trace(go.Bar(
        x=pos, y=vol, name='成交量', marker_color=vol_colors,
        hovertemplate='<b>%{customdata}</b> 量 %{y:.0f}<extra></extra>',
        customdata=time_labels), row=2, col=1)

    # 全天最低标注
    if 'low' in df.columns:
        lo = df.loc[df['low'].idxmin()]
        lo_pos = pos[df.index.get_loc(lo.name)]
        fig.add_annotation(x=lo_pos, y=lo['low'],
                           text=f"最低 {lo['low']:.2f}",
                           showarrow=True, arrowhead=2, arrowcolor='red',
                           font=dict(color='red', size=13), row=1, col=1)

    # 刻度只标整点/半点的交易时刻（约每30分钟一个）
    tick_step = 30
    tickvals = list(range(0, len(pos), tick_step))
    if tickvals[-1] != len(pos) - 1:
        tickvals.append(len(pos) - 1)
    ticktext = [time_labels[i] for i in tickvals]

    st = title or f"{stock_code} {stock_name} 分时走势  {df.index[0]:%Y-%m-%d}  昨收 {prev_close:.2f}"
    fig.update_layout(
        title=dict(text=st, x=0.02),
        xaxis_rangeslider_visible=False, hovermode='x unified',
        template='plotly_white', height=700,
        legend=dict(orientation='h', yanchor='bottom', y=1.02, x=0),
        margin=dict(l=20, r=20, t=60, b=20), uirevision='constant',
        transition=dict(duration=0), autosize=False)
    fig.update_xaxes(tickmode='array', tickvals=tickvals, ticktext=ticktext,
                     showgrid=True, gridcolor='rgba(200,200,200,0.3)', row=1, col=1)
    return fig
