...
def _sep_image(w):
    """分隔渐变图: 中间实->两端融进面板底(不缓存: PhotoImage 不能跨Tk解释器线程用)"""
...
def _rounded_bg(w, h, r, corner=None):
    ...
    corner = _TC if corner is None else corner
...
def show_popup():
    """托盘右键: 在鼠标处弹自绘圆角菜单(独立线程, 系统菜单无法圆角/渐变)"""
    threading.Thread(target=_popup_run, daemon=True).start()
...
def _popup_run():
    ...
    tc_ok = False
    try:
        # Windows Tk: 指定色像素全透明 -> 用于把窗口四角切成斜切倒角(窗框外形倒角)
        root.attributes("-transparentcolor", _TC)
        tc_ok = True
    except Exception:
        pass
    ...
    cv = tk.Canvas(root, width=W, height=H, bg=_TC if tc_ok else _BG,
                   highlightthickness=0, bd=0)
    cv.pack()

    sep_img = _sep_image(W - 2 * IN)
    menu_bg = _rounded_bg(W, H, _R, corner=_TC if tc_ok else _BG)
    hover_row = [None]

    def draw(h):
        cv.delete("all")
        # 整窗圆角面板底(PIL 预渲染: 四角为圆弧, 圆角外=_TC 真透明)
        cv.create_image(0, 0, image=menu_bg, anchor="nw")
        for r, y0, y1 in geo:
            if r.get("sep"):
                sx, sy = IN, (y0 + y1) // 2
    ...
    def poll_outside():
        ...
        if not root.winfo_exists():
            return
        if time.time() - opened[0] > 0.4:
            try:
                if ctypes.windll.user32.GetAsyncKeyState(1) & 1:
...
def _panel_run():
    """在独立线程建 Tk 面板: 大彩灯(Canvas 真色圆) + 状态 + 操作按钮, 每1秒刷新"""
    global _panel_root
    try:
        import tkinter as tk
    except Exception as e:
    ...
    try:
        ...
        tc_ok_p = False
    sw, sh = root.winfo_screenwidth(), root.winfo_screenheight()
    root.geometry(f"360x240+{sw - 400}+{sh - 330}")
    bg = _BG                             # 与右键菜单同一面板色
    root.configure(bg=_TC if tc_ok_p else bg)
    # 最底层: 圆角底图(圆角外=透明色 -> 窗口真圆角)
    bg_cv = tk.Canvas(root, width=360, height=240,
                      bg=_TC if tc_ok_p else bg, highlightthickness=0, bd=0)
    bg_cv.place(x=0, y=0)
    bg_img = _rounded_bg(360, 240, _PANEL_R, corner=_TC if tc_ok_p else bg)
    bg_cv.create_image(0, 0, image=bg_img, anchor="nw")
    # 内容壳: 内缩 R, 让四角弧形区域留在底图层
    shell = tk.Frame(root, bg=bg)
    shell.place(x=_PANEL_R, y=_PANEL_R,
                width=360 - 2 * _PANEL_R, height=240 - 2 * _PANEL_R)

    # 顶栏(按住可拖动; ✕关闭面板)
    bar = tk.Frame(shell, bg=_hex(GREEN), height=34)
    bar.pack(fill="x")
    bar.pack_propagate(False)
    tk.Label(bar, text="量化看守", bg=_hex(GREEN), fg="white",
             font=("Microsoft YaHei UI", 11, "bold")).pack(side="left", padx=10)
    close_lab = tk.Label(bar, text="✕", bg=_hex(GREEN), fg="white",
    ...
    def _btn(txt, fn):
        b = tk.Button(btns, text=txt, command=fn, bg="#ffffff", fg=_FG,
                      relief="flat", bd=0, highlightthickness=1,
                      highlightbackground=_OUT, activebackground=_HOV,
                      font=("Microsoft YaHei UI", 9))
        b.pack(side="left", expand=True, fill="x", padx=3)
        b.bind("<Enter>", lambda e: b.config(bg=_HOV))
        b.bind("<Leave>", lambda e: b.config(bg="#ffffff"))
...
def _patch_rightclick_popup():
    """把 pystray 的右键弹系统菜单 换成 自绘圆角菜单(系统菜单无法圆角/渐变)"""
    try:
        from pystray import _win32 as _pw
        _orig = _pw.Icon._on_notify

        def wrap(self, wparam, lparam):
            if lparam == 0x0205:  # WM_RBUTTONUP
                try:
                    show_popup()
                except Exception as e:
                    tlog(f"popup err: {repr(e)[:80]}")
                return None
        ...
        tlog(f"patch_rightclick err: {repr(e)[:80]}")
        return False


_PATCH_RC = _patch_rightclick_popup()
